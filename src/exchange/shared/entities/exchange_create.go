...
func (ec *ExchangeCreate) check() error {
	if _, ok := ec.mutation.ExchangeID(); !ok {
		return &ValidationError{Name: "exchange_id", err: errors.New(`entities: missing required field "Exchange.exchange_id"`)}
	}
	return nil
}
...
func (u *ExchangeUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("entities: missing options for ExchangeCreate.OnConflict")
	}
...
func (u *ExchangeUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("entities: OnConflict was set for builder %d. Set it on the ExchangeCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("entities: missing options for ExchangeCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
