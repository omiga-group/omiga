...
func (f ExchangeFunc) Mutate(ctx context.Context, m repositories.Mutation) (repositories.Value, error) {
	mv, ok := m.(*repositories.ExchangeMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *repositories.ExchangeMutation", m)
	}
	return f(ctx, mv)
}
