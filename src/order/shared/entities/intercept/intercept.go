...
type OrderFunc func(context.Context, *entities.OrderQuery) (entities.Value, error)

// Query calls f(ctx, q).
func (f OrderFunc) Query(ctx context.Context, q entities.Query) (entities.Value, error) {
	if q, ok := q.(*entities.OrderQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *entities.OrderQuery", q)
}

// The TraverseOrder type is an adapter to allow the use of ordinary function as Traverser.
type TraverseOrder func(context.Context, *entities.OrderQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseOrder) Intercept(next entities.Querier) entities.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseOrder) Traverse(ctx context.Context, q entities.Query) error {
	if q, ok := q.(*entities.OrderQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *entities.OrderQuery", q)
}
...
func NewQuery(q entities.Query) (Query, error) {
	...
	case *entities.OrderQuery:
		return &query[*entities.OrderQuery, predicate.Order]{typ: entities.TypeOrder, tq: q}, nil
