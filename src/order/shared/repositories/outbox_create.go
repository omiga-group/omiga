...
func (oc *OutboxCreate) check() error {
	if _, ok := oc.mutation.Timestamp(); !ok {
		return &ValidationError{Name: "timestamp", err: errors.New(`repositories: missing required field "Outbox.timestamp"`)}
	}
	if _, ok := oc.mutation.Topic(); !ok {
		return &ValidationError{Name: "topic", err: errors.New(`repositories: missing required field "Outbox.topic"`)}
	}
	if _, ok := oc.mutation.Key(); !ok {
		return &ValidationError{Name: "key", err: errors.New(`repositories: missing required field "Outbox.key"`)}
	}
	if _, ok := oc.mutation.Payload(); !ok {
		return &ValidationError{Name: "payload", err: errors.New(`repositories: missing required field "Outbox.payload"`)}
	}
	if _, ok := oc.mutation.Headers(); !ok {
		return &ValidationError{Name: "headers", err: errors.New(`repositories: missing required field "Outbox.headers"`)}
	}
...
func (u *OutboxUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("repositories: missing options for OutboxCreate.OnConflict")
	}
...
func (u *OutboxUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("repositories: OnConflict was set for builder %d. Set it on the OutboxCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("repositories: missing options for OutboxCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
