...
type OrderQueryRuleFunc func(context.Context, *entities.OrderQuery) error

// EvalQuery return f(ctx, q).
func (f OrderQueryRuleFunc) EvalQuery(ctx context.Context, q entities.Query) error {
	if q, ok := q.(*entities.OrderQuery); ok {
		return f(ctx, q)
	}
	return Denyf("entities/privacy: unexpected query type %T, expect *entities.OrderQuery", q)
}

// The OrderMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type OrderMutationRuleFunc func(context.Context, *entities.OrderMutation) error

// EvalMutation calls f(ctx, m).
func (f OrderMutationRuleFunc) EvalMutation(ctx context.Context, m entities.Mutation) error {
	if m, ok := m.(*entities.OrderMutation); ok {
		return f(ctx, m)
	}
	return Denyf("entities/privacy: unexpected mutation type %T, expect *entities.OrderMutation", m)
}
