...
// OrderBookEdge is the edge representation of OrderBook.
type OrderBookEdge struct {
	Node   *OrderBook `json:"node"`
	Cursor Cursor     `json:"cursor"`
}

// OrderBookConnection is the connection containing edges to OrderBook.
type OrderBookConnection struct {
	Edges      []*OrderBookEdge `json:"edges"`
	PageInfo   PageInfo         `json:"pageInfo"`
	TotalCount int              `json:"totalCount"`
}

func (c *OrderBookConnection) build(nodes []*OrderBook, pager *orderbookPager, after *Cursor, first *int, before *Cursor, last *int) {
	c.PageInfo.HasNextPage = before != nil
	c.PageInfo.HasPreviousPage = after != nil
	if first != nil && *first+1 == len(nodes) {
		c.PageInfo.HasNextPage = true
	...
	var nodeAt func(int) *OrderBook
	if last != nil {
		n := len(nodes) - 1
		nodeAt = func(i int) *OrderBook {
			return nodes[n-i]
		}
	} else {
		nodeAt = func(i int) *OrderBook {
	...
	c.Edges = make([]*OrderBookEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		c.Edges[i] = &OrderBookEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
...
// OrderBookPaginateOption enables pagination customization.
type OrderBookPaginateOption func(*orderbookPager) error

// WithOrderBookOrder configures pagination ordering.
func WithOrderBookOrder(order *OrderBookOrder) OrderBookPaginateOption {
	if order == nil {
		order = DefaultOrderBookOrder
	}
	o := *order
	return func(pager *orderbookPager) error {
		if err := o.Direction.Validate(); err != nil {
			return err
		}
		if o.Field == nil {
			o.Field = DefaultOrderBookOrder.Field
		}
		pager.order = &o
		return nil
	}
...
// WithOrderBookFilter configures pagination filter.
func WithOrderBookFilter(filter func(*OrderBookQuery) (*OrderBookQuery, error)) OrderBookPaginateOption {
	return func(pager *orderbookPager) error {
		if filter == nil {
			return errors.New("OrderBookQuery filter cannot be nil")
		}
		pager.filter = filter
		return nil
	}
...
type orderbookPager struct {
	...
	filter func(*OrderBookQuery) (*OrderBookQuery, error)
}

func newOrderBookPager(opts []OrderBookPaginateOption) (*orderbookPager, error) {
	pager := &orderbookPager{}
	for _, opt := range opts {
		if err := opt(pager); err != nil {
			return nil, err
		}
	...
	if pager.order == nil {
		pager.order = DefaultOrderBookOrder
	}
	return pager, nil
}

func (p *orderbookPager) applyFilter(query *OrderBookQuery) (*OrderBookQuery, error) {
	if p.filter != nil {
		return p.filter(query)
	}
	return query, nil
...
func (p *orderbookPager) toCursor(ob *OrderBook) Cursor {
	return p.order.Field.toCursor(ob)
}

func (p *orderbookPager) applyCursors(query *OrderBookQuery, after, before *Cursor) *OrderBookQuery {
	for _, predicate := range cursorsToPredicates(
		p.order.Direction, after, before,
		p.order.Field.field, DefaultOrderBookOrder.Field.field,
	) {
		query = query.Where(predicate)
	}
	return query
...
func (p *orderbookPager) applyOrder(query *OrderBookQuery, reverse bool) *OrderBookQuery {
	direction := p.order.Direction
	if reverse {
		direction = direction.reverse()
	}
	query = query.Order(direction.orderFunc(p.order.Field.field))
	if p.order.Field != DefaultOrderBookOrder.Field {
		query = query.Order(direction.orderFunc(DefaultOrderBookOrder.Field.field))
	}
	return query
}

func (p *orderbookPager) orderExpr(reverse bool) sql.Querier {
	direction := p.order.Direction
	if reverse {
		direction = direction.reverse()
	}
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.field).Pad().WriteString(string(direction))
		if p.order.Field != DefaultOrderBookOrder.Field {
			b.Comma().Ident(DefaultOrderBookOrder.Field.field).Pad().WriteString(string(direction))
		}
	})
}

// Paginate executes the query and returns a relay based cursor connection to OrderBook.
func (ob *OrderBookQuery) Paginate(
	ctx context.Context, after *Cursor, first *int,
	before *Cursor, last *int, opts ...OrderBookPaginateOption,
) (*OrderBookConnection, error) {
	if err := validateFirstLast(first, last); err != nil {
		return nil, err
	}
	pager, err := newOrderBookPager(opts)
	if err != nil {
		return nil, err
	}
	if ob, err = pager.applyFilter(ob); err != nil {
		return nil, err
	}
	conn := &OrderBookConnection{Edges: []*OrderBookEdge{}}
	ignoredEdges := !hasCollectedField(ctx, edgesField)
	if hasCollectedField(ctx, totalCountField) || hasCollectedField(ctx, pageInfoField) {
		hasPagination := after != nil || first != nil || before != nil || last != nil
		if hasPagination || ignoredEdges {
			if conn.TotalCount, err = ob.Count(ctx); err != nil {
				return nil, err
			}
			conn.PageInfo.HasNextPage = first != nil && conn.TotalCount > 0
			conn.PageInfo.HasPreviousPage = last != nil && conn.TotalCount > 0
	...
	ob = pager.applyCursors(ob, after, before)
	ob = pager.applyOrder(ob, last != nil)
	if limit := paginateLimit(first, last); limit != 0 {
		ob.Limit(limit)
	}
	if field := collectedField(ctx, edgesField, nodeField); field != nil {
		if err := ob.collectField(ctx, graphql.GetOperationContext(ctx), *field, []string{edgesField, nodeField}); err != nil {
			return nil, err
		}
	}

	nodes, err := ob.All(ctx)
	if err != nil {
		return nil, err
	}
	conn.build(nodes, pager, after, first, before, last)
...
// OrderBookOrderField defines the ordering field of OrderBook.
type OrderBookOrderField struct {
	field    string
	toCursor func(*OrderBook) Cursor
}

// OrderBookOrder defines the ordering of OrderBook.
type OrderBookOrder struct {
	Direction OrderDirection       `json:"direction"`
	Field     *OrderBookOrderField `json:"field"`
}

// DefaultOrderBookOrder is the default ordering of OrderBook.
var DefaultOrderBookOrder = &OrderBookOrder{
	Direction: OrderDirectionAsc,
	Field: &OrderBookOrderField{
		field: orderbook.FieldID,
		toCursor: func(ob *OrderBook) Cursor {
			return Cursor{ID: ob.ID}
		},
	},
}

// ToEdge converts OrderBook into OrderBookEdge.
func (ob *OrderBook) ToEdge(order *OrderBookOrder) *OrderBookEdge {
	if order == nil {
		order = DefaultOrderBookOrder
	}
	return &OrderBookEdge{
		Node:   ob,
		Cursor: order.Field.toCursor(ob),
	}
}
