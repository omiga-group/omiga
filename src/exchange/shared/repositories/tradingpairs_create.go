...
func (tpc *TradingPairsCreate) check() error {
	if _, ok := tpc.mutation.Symbol(); !ok {
		return &ValidationError{Name: "symbol", err: errors.New(`repositories: missing required field "TradingPairs.symbol"`)}
	}
	if _, ok := tpc.mutation.Base(); !ok {
		return &ValidationError{Name: "base", err: errors.New(`repositories: missing required field "TradingPairs.base"`)}
	}
	if _, ok := tpc.mutation.BasePrecision(); !ok {
		return &ValidationError{Name: "base_precision", err: errors.New(`repositories: missing required field "TradingPairs.base_precision"`)}
	}
	if _, ok := tpc.mutation.Counter(); !ok {
		return &ValidationError{Name: "counter", err: errors.New(`repositories: missing required field "TradingPairs.counter"`)}
	}
	if _, ok := tpc.mutation.CounterPrecision(); !ok {
		return &ValidationError{Name: "counter_precision", err: errors.New(`repositories: missing required field "TradingPairs.counter_precision"`)}
	}
	if _, ok := tpc.mutation.ExchangeID(); !ok {
		return &ValidationError{Name: "exchange", err: errors.New(`repositories: missing required edge "TradingPairs.exchange"`)}
	}
	return nil
}
...
func (u *TradingPairsUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("repositories: missing options for TradingPairsCreate.OnConflict")
	}
...
func (u *TradingPairsUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("repositories: OnConflict was set for builder %d. Set it on the TradingPairsCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("repositories: missing options for TradingPairsCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
